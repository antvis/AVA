...
1. Download both dataset variants with `pnpm eval databench:fetch` (no parameters).
2. Configure the model in `.env`:
...
3. Run `pnpm eval databench`. Available options:
   ...
   - `--output <path>` sets the result CSV path (default: `evals/databench/results/<dataset>.csv`).

4. AVA generates and executes SQL, scores the answer, and saves the result CSV.

### Prediction files

A prediction file is a CSV that maps each dataset question ID to AVA's answer. `pnpm eval databench` creates one automatically at `evals/databench/results/<dataset>.csv` unless `--output` sets another path.
...
```bash
pnpm eval --predictions <file.csv>
```
