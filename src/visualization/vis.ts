/**
 * Chart definitions and advisor prompt builder
 *
 * Structured chart type definitions used to dynamically build prompts
 * for the chart advisor and for visualization generation.
 */
...
export const allChartTypes = chartDefinitions.map(d => d.type);

export const chartTypeList = [...allChartTypes, 'none'].map(t => `"${t}"`).join(', ');

/**
 * Build the advisor prompt dynamically from chartDefinitions
 */
export function buildAdvisorPrompt(query: string, dataInfoStr: string): string {
