...
export async function recommendChartIds(params: {
  ...
  const prompt =
    Array.isArray(query) && !isEmpty(query)
      ? adviseChartByDataShardPrompt(
          query.map(({ metas, data, purpose }) => ({
      ...
      : adviseChartByInputPrompt([query as string], allowedIds);
...
export async function generateSpecs(params: {
  ...
  const prompt =
    Array.isArray(query) && !isEmpty(query)
      ...
      : genSpecByInputPrompt([{ input: query as string, chartId: selectedChartIds[0] }]);
...
export async function adviseCharts(query: DataShard[] | string, config: AVAContext): Promise<AdviseStageOutput> {
  ...
  const dataShards = Array.isArray(query) ? (query as DataShard[]) : [];
