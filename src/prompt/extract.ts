...
\`\`\`typescript
  ...
  type DataShard = {
    ...
    metas: Array<{
      ...
      unit?: string; // 语意单位，表示数据的量纲。对于 dataType 为 'number' 的字段，此项为必填。
    }>; // 数据描述信息，包括各字段的名称类型
    purpose?: {
      name: string;
      key: string;
...
- 对于所有 \`dataType\` 为 \`'number'\` 的字段，其 \`meta\` 信息中必须包含 \`unit\` 字段。如果文本中明确提供了单位（如“100人”），则使用该单位（“人”）。如果文本中没有提供明确的单位（如“销量为100”），则应根据上下文推断一个合理的单位，如果无法推断，则默认为“个”。
...
type DataShard = {
  ...
  metas: Array<{
    ...
    unit?: string; // Semantic unit, e.g., "person", "yuan", etc. This field is mandatory for fields with dataType 'number'.
...
- For all fields with \`dataType\` of \`'number'\`, their \`meta\` information must include the \`unit\` field. If a unit is explicitly provided in the text (e.g., "100 people"), use that unit ("people"). If no explicit unit is provided (e.g., "sales volume is 100"), infer a reasonable unit from the context. If it cannot be inferred, default to "individual".
