...
const extractHomogeneousPatterns = (collection: ScopePatternCollection, type: InsightType): HomogeneousPatternInfo[] => {
  ...
  if (validScopes.length > 1) {
    if (type === 'trend') {
      ...
      if (parts.length === 2 && parts[0][0]?.patterns?.length && (parts[0].length / scopeLength > 0.75) && parts[1].length < 5) {
    ...
    if (['change_point', 'outlier', 'time_series_outlier'].includes(type)) {
      const commSetIndexes = Object.values(_groupBy(_flatten(validScopes.map(item => item.patterns.map((item: PointPatternInfo) => item.index))))).sort((a, b) => b.length - a.length);
      commSetIndexes.forEach((indexArr) => {
        const ratio = indexArr.length / scopeLength;
        if (ratio > 0.3 && indexArr.length >= 3) {
          const scopes = validScopes.filter(item => item.patterns.some((item: PointPatternInfo) => item.index === indexArr[0]));
          const childPatterns = _flatten(scopes.map(item => item.patterns.filter((item: PointPatternInfo) => item.index === indexArr[0])));
          homogeneousPatterns.push({
            type: 'commonness',
            insightType: type,
            childPatterns,
...
export const extractHomogeneousPatternsForMeausres = (measures: Measure[], patternsArr: PatternCollection[]): HomogeneousPatternInfo[] => {
...
export const extractHomogeneousPatternsForSiblingGroups = (measures: Measure[], siblingItems: string[], insightsCollection: InsightInfo<PatternInfo>[][]): HomogeneousPatternInfo[][] => {
  ...
  for(let i = 0; i < measuresLength; i += 1) {
    ...
    PATTERN_TYPES.forEach(type => {
      const patternCollection = patternsArr.map((arr, index) => ({ key: siblingItems[index], patterns: arr?.filter(item => item.type === type)}));
      const patterns = extractHomogeneousPatterns(patternCollection, type);
