...
export const matchGraph: MatchFunction = (data: any) => {
  ...
  const standardizedNodes = nodes.map((node) => {
    ...
    const id = idField ? String(node[idField]) : generateFallbackId(node);
  ...
  const standardizedEdges = edges.map((edge) => {
    ...
    const source = sourceField ? String(edge[sourceField]) : '';
    const target = targetField ? String(edge[targetField]) : '';
  ...
  const hasValidNodes =
    ...
    standardizedNodes.every(
      (node) => typeof node === 'object' && node !== null && typeof node.id === 'string' && node.id.trim() !== ''
    );
