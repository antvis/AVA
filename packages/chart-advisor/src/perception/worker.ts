...
export const perceptionIW: Worker = function(data: RowData[]): Insight[] {
  ...
  for (let i = 0; i < scagOut.length; ++i) {
    ...
    const subData = columnsToRowData(
      [columns[fixnum[scagOut[i].indX!]], columns[fixnum[scagOut[i].indY!]]],
      [dimensionTitle, measureTitle]
    );

    insights.push({
      type: 'Perception',
      description: `Perceptual insight with '${SCAG_TYPES[scagOut[i].k!]}'`,
      fields: [
        columnProps[fixnum[scagOut[i].indX!]].title as string,
